...
export interface ChannelCapabilities {
  ...
  /**
   * 这个渠道的凭据是否**关在我们自己的目录里**（可以安全地退登/换账号）。
   *
   * ## ★★ 为什么这必须是一个能力字段，而不是在 UI 里判渠道 id
   *
   * 两个渠道在这件事上**物理上就不同**：
   * · 钉钉：token 的加密密钥在**系统钥匙串**、按系统用户存一份，也就是与用户
   *   自己终端里的 CLI **共用同一份登录态**（实测确认）。我们去退登会把用户
   *   终端里正在用的那份也退掉 —— 所以界面上刻意**不提供**退出授权按钮，
   *   只给一句"要退请自己在终端退"的说明；
   * · 飞书：`HOME`/配置目录/master key 全被重定向进 `<vault>/channels/feishu/`，
   *   且 `keychain-downgrade` 明确不碰系统钥匙串。凭据**只属于这个 vault**，
   *   退它不影响任何别的地方 —— 所以可以、也应该给按钮。
   *
   * 把它做成 `isolatedCredentials` 之后，"给不给退登/切账号按钮"这个判据
   * 有了单一真源：新渠道接进来时它自己声明，UI 不需要认识任何渠道名字
   * （这批多渠道重构一直在消除的正是"UI 里散落 `id === "dingtalk"`"那种写法）。
   */
...
export interface ChannelAuth {
  ...
  /**
   * 为「切换账号 / 换 app」做准备：把**凭据与 app 绑定一起**清掉。
   * **渠道不支持时可省略**（那时界面只提供「退出授权」，不提供「切换账号」）。
   *
   * ## ★ 与 `logout` 的区别（这不是同一件事）
   *
   * `logout` 只清 token。而有些渠道的 CLI 会把"用哪个 app 授权"也记在配置里
   * —— 只清 token 的话，下一次 `login()` 仍用那个已绑定的 app 与**同一个
   * 账号**应答，于是用户点多少次「重新授权」都换不了人（飞书实测症状）。
   *
   * 所以「切换账号」= 清 token + 清 app 配置 → 再走一次 `login()`，
   * 那时渠道 CLI 才会重新问"用哪个 app / 谁来授权"。
   *
   * 破坏性比 `logout` 更强，**只在用户显式选择"切换账号"时调**。
   * 返回是否真的清掉了；**不抛**（与 `logout` 同一条纪律）。
   */
  resetForAccountSwitch?(): Promise<boolean>
}
