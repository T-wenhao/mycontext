...
规划里没有这两条 —— 它们是用户看着**真实数据**报出来的，
而两个都不是"数据错了"，是**数据对而显示在说谎**。

### 13.1 G10：文档覆盖面把空间维度的事实说成时间进度

用户原话：

> 文档：2026-05-18 起已有 291 篇，覆盖 62 天。
> 其中 **0 天已采完**，62 天还在往回补。

**根因有两层。**

第一层：`drained` 原来取 `!listed.truncated`，而那个布尔的四个来源里有一个
是「知识库整段不可用（没开通/无权限）」的 `catch`，**每轮都命中**。
实测本机 vault：`document_coverage` **453 行全部 `drained=0`**，一行 1 都没有。

第二层（真正的那一层）是**粒度错配**：`incomplete` 是**整轮列举**的属性，
而 `drained` 存在每个 (空间, 天) 上。也就是说文档的 `drainedDays` 只有两种
取值 —— 0 或 dayCount —— 它**完全由 `incomplete` 决定**，一个字节的额外信息
都没有。而文档是**按空间**翻页的（`wiki node list` 递归 + drive 游标），
一天压根不存在"翻完"这件事。那个 62 是空间维度的事实被伪装成了时间进度，
用户会等一个不存在的进度条走完。
...
| 层     | 改动                                                                                                                                            | 判据                                                                                   |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| 渠道   | `DocumentListIncomplete` 三态 + `noteIncomplete()` 按严重度收敛                                                                                 | `unavailable` 压过另两个                                                               |
| 主进程 | `IngestService.documentsIncompleteReason` getter → `DataPlaneService.documentsIncomplete(channelId)` → `startup.ts` 注入 `DistillSourceService` | 那个事实只在内存里（刻意不落库：存下来会过期，而过期的方向是显示一个早已解除的不可用） |
| 界面   | 文档域三支话**一个"天"字都不提**                                                                                                                | `unavailable` 说出路；另两种说"还在列"；`null` 说"能列到的都列完了"                    |

★ **库这一层保持保守**（如实记 `drained=0`，不假装采完），由界面负责不把它
说成时间维度的进度。反过来（库里写 1 装作采完）是在库里制造一个谎。

**★★ 一处我先写错、被反证抓出来的判断。** 我在 `ingest.service.ts` 的注释里
把"换成 `incomplete === null`"写成了这个缺陷的修法。实测反证：把判据改回
`!listed.truncated` 之后**五条用例全绿** —— 因为渠道那侧三处截断站点是
`truncated = true` 与 `noteIncomplete(...)` **成对**设置的，两个表达式恒等。
换判据的真实理由只是"`incomplete` 带成因、`truncated` 是它的有损投影"，
注释已按实测改写。

**★★ 一个已知的测试缺口（不掩盖）。** `noteIncomplete` 里
`if (next === "unavailable" || incomplete === null)` 的**前半段**在当前实现下
**跑不到**，所以它没有反证 —— 删掉那半句全部用例仍然绿。原因是控制流：
`unavailable` 只在 `listSpaces()` 自己抛时命中，而它一抛就走不到
`spaces.hasMore` 与逐空间的 `nodes.truncated`（那两处都在同一个 try 里、
在它之后）；而 `listWikiNodes` 对单个文件夹失败是 `continue` 不抛。
也就是说三种成因今天**互斥**。那半句是防御性的，理由与"以后有人把
`listWikiNodes` 的失败改成往上抛"写在 `documents.test.ts` 里。
留着它 + 写清缺口，比写一条恒绿的用例好 —— 恒绿的用例会让下一个人
以为这条判据有人守着。

**CDP 实测（真应用、真 vault）：**

```
改动前（反证：把 domain === "doc" 判据去掉）：
  文档：2026-05-18 起已有 269 篇，覆盖 60 天 | 其中 0 天已采完，60 天还在往回补。
改动后：
  文档：2026-05-18 起已有 269 篇，覆盖 60 天 | 能列到的文档都已列完。
```

探针加了一条判据（`docSentence`）：**先切出文档那一句再判**有没有天数对。
直接在整段 body 上判会恒绿 —— 聊天/听记那两行**合法地**在用"往回补"。

### 13.2 G11：头像失败的归因错 + 整轮同因还试 15 遍

用户日志：17 秒里 15 次 `listGroupMembersByUids error: Decode parameter error: 2`
（`server_error_code: 1001` / `server_key: im`）。**两个独立缺陷。**

**① `1001` 被当成"保密群"。** 上游把这个码复用成了通用 im 错误码
（已知三种含义：保密群 / `org not match` / 参数解不开）。
`Decode parameter error` 是**我们自己传错参数**，而它落到
`RESOURCE_FORBIDDEN` 之后 `mediaIdFromGroup` 的 catch **静默跳过**这个群
→ `group_unreadable` → 界面记成可重试的 `failed`。
于是一个我们能自己修的 bug 被永久记成"对方的保密群"，
排查的人看日志只看到"保密群"。

修法：新增 `CHANNEL_BAD_PARAMETER`（仍是终态，但**归因指向我们**）。
两个码的修法在**不同的人**身上 —— 合并成一个就等于把自己的 bug 藏进
对方的权限墙后面。★ `ERROR_CODES` 是值而不只是类型，所以 i18n 那条门禁
立刻报了缺译文（zh/en 各一条），那正是它存在的理由。

**② 整轮性的墙也把每个人试一遍。** 每个人 2-3 次子进程、串行跑。
而 `not_permitted`（这份客户端对这个企业没开通该能力）对第 1 个人成立
就对第 60 个人成立 —— 同一个 OAuth 客户端、同一个企业。继续循环的收益
恒为零，代价是几十次子进程 + 十几秒卡顿，用户看到的只是
"刷新头像转了很久然后什么都没变"。

判据是「这个失败是**整轮**的属性还是**这个人**的属性」，
**不是**"它有多严重"：

- `not_permitted` → 整轮属性，**停**；
- `failed` → 可重试（子进程超时、网络抖动），确实可能只影响一个人，
  **不停**。对它短路会把一次抖动变成"整批放弃" —— 而那正是
  `mediaAvatarsFetch` 的注释里说要避免的"一个人的超时带走整批"；
- 其余三个（`not_set` / `not_reachable` / `not_attempted`）压根不是故障。

★ 把 `AVATAR_REASON_PRIORITY` / `worseAvatarReason` / `isWholeRoundAvatarWall`
从 IPC handler 的闭包里提到模块级导出 —— 那张表的**顺序就是判据**，
而它原来一条用例都碰不到。跳过的人**不计入 `failed`**（我们没试过，
报成失败会让"取不到几个"失真），但要 `logger.info` 留痕：
静默少跑几十次会让下一个排查的人以为那些人被试过且成功了。

**★★★ 这一条没能在真机上复现。** 本机随包渠道客户端的登录态已过期 ——
CDP 实测每条渠道命令都在 30s 超时（`selfAvatar` 返回 `reason: "failed"`，
日志里是 `进程超时（30000ms）已终止`），所以**权限墙这条路走不到**。
已验证的是：`not_attempted`（缺花名）确实**不**触发短路（真机 7ms 返回，
一次子进程都没起），以及九条单测 + 三个反证（判据恒 false / 放宽成
`reason !== null` / 优先级取反）都按预期转红。

`Decode parameter error: 2` 里那个 `2` 大概是出错的参数序号 —— **未证实**，
没有可复现的环境，所以原样留在 `context` 里给日志，不猜。
要定死它的成因需要用户那份自备客户端，或本机重新登录。

### 13.3 §12.7 记的三处既存问题：这一轮**一处都没修**（都不用修了）

★ 这一节原来写的是"顺带修掉了两处"，**那是错的**，改成实测结论 ——
写在这里而不是删掉，因为"以为自己修了一个早已被别人修掉的问题"
是这类跨轮次文档最容易长出来的谎。

- `settings-view.tsx` 那个不存在的 `typography-body-small-500`：
  **上游已用另一条路修掉了**（`29d7a101`）。现在那一行写的是
  `typography-body-small-400 font-medium` —— 同字号靠 Tailwind 的
  `font-medium` 加粗，而不是给表加一档。那个做法更好：
  「同字号 + 强调」在状态面板/图谱面板里也是这么写的，
  给表加第 16 档等于给同一件事开第二条路。
  ★ 本轮**试过**加 `.typography-body-small-500`，随后回退 ——
  加完之后它一个使用方都没有，而 `check-typography-classes.mjs`
  **不查未使用的类**，所以那次它绿了并不代表那一档是对的；
- `kl-server.service.ts` 两条 `no-useless-assignment`：HEAD 上
  `npx eslint` 已经干净（退出码 0），同样在 `29d7a101` 里被修掉了；
- `vendor/` 下的 `__pycache__` 是**跑应用/测试的副产物**（每次跑完又会
  长出来），不是仓库内容 —— `find vendor -name __pycache__ -exec rm -rf {} +`。

### 13.4 门禁

```
pnpm run test   → 285 文件 / 4934 条，**4933 绿 + 1 红**
tsc -b / eslint / check:all / check:no-local-data / check:trademarks → 全绿
                  （no-local-data 在**有真实 vault** 的机器上跑的：
                   11725 个已跟踪文件比对 76326 个真实值）
CDP 探针（真应用 + 真 vault）→ 四类断言 + 新增的文档那一句判据全部成立
                  （★ 这一行是**改动当轮**跑的；提交前的复核只跑了上面几项，
                   探针要带 `--remote-debugging-port` 重起应用，没有重跑）
```

★★ 那 1 条红的是 `tests/unit/dingtalk-auth.test.ts`「显式启用推荐权限，
并依次打开登录页与授权范围页」（`CHANNEL_AUTH_FAILED`：授权流程结束但
未检测到有效登录态）。`git stash -u` 到干净 HEAD 后**同样失败** ——
既存问题，与这两轮无关，这一轮没处理。

★ `typecheck:tests`（`tests/tsconfig.json`，比主 tsconfig 严）有 **120 条**
既存报错，`git stash` 后**同样 120 条**，且没有一条在这两轮改动的文件里
—— 那是这个分支上原有的，没在这一轮处理。
