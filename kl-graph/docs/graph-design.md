...
| **Fact** | A reified claim/event extracted once from an extraction item and projected onto one or more evidence chunks. `source_chunk_id` is the primary chunk; additional provenance is represented by `STATES`. Typed by `fact_type` (open vocabulary). The nullable subject/object IDs preserve participant roles in addition to the role-neutral `ABOUT` edges. | `id`, `text`, `fact_type`, `timestamp`, `confidence`, `subject_entity_id`, `object_entity_id`, `source_chunk_id`, `source_unit_id`, `extraction_item_id` |
...
| Fact | `fact_type` | `DECISION`, `DELEGATE`, `REQUEST`, `ACTION_ITEM`, `STATUS`, `CAUSAL`, `OPINION`, `GENERAL` |
...
`REQUEST` represents an information-seeking or conversational ask that can be
resolved by a simple reply. `ACTION_ITEM` represents a concrete requested or
assigned action with an observable completion result. Their participant roles
are fixed: `subject_entity_id` is the requester and `object_entity_id` is the
recipient/assignee. Both are extracted regardless of recipient; filtering for
facts addressed to a configured current user is a query-time operation. An ask
addressed to multiple recipients is stored as one fact per recipient.
`DELEGATE` remains for established ownership or division of work rather than
the speech act of asking. Other fact types may also populate the participant
IDs when the roles are explicit.
